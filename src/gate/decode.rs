...
fn is_base64_decode_stage(stage: &str) -> bool {
    ...
    while is_wrapper(head) || is_env_var_assignment(head) {
...
fn is_wrapper(token: &str) -> bool {
    ...
    matches!(
        bin_name,
        "env" | "sudo" | "stdbuf" | "time" | "command" | "builtin" | "exec"
    )
}
...
mod tests {
    ...
    #[test]
    fn decodes_env_var_prefix_port91() {
        // Port from #91: `FOO=1 base64 -d` must still be detected.
        let leg = "echo cm0gLXJmIH4K | FOO=1 base64 -d | sh";
        let out = decode_and_expand(leg, 0).expect("decode FOO=1");
        assert_eq!(out.trim(), "rm -rf ~");
        assert!(has_base64_decode_stage("FOO=1 base64 -d"));
    }

    #[test]
    fn decodes_builtin_exec_wrappers_port91() {
        // Port from #91: `builtin`/`exec` are wrappers like `env`/`sudo`.
        for wrapper in ["builtin", "exec"] {
            let leg = format!("echo cm0gLXJmIH4K | {wrapper} base64 -d | sh");
            let out = decode_and_expand(&leg, 0).expect("decode wrapper");
            assert_eq!(out.trim(), "rm -rf ~", "wrapper {wrapper} failed");
        }
    }

    #[test]
    fn decodes_long_decode_eq_port91() {
        // Port from #91: `--decode=<arg>` form must count as decode flag.
        let leg = "echo cm0gLXJmIH4K | base64 --decode=x | sh";
        let out = decode_and_expand(leg, 0).expect("decode --decode=");
        assert_eq!(out.trim(), "rm -rf ~");
    }
