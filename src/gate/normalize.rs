...
pub struct Normalized<'a> {
    ...
    pub extra_separators: SmallVec<[char; 2]>,
}
...
pub fn normalize_command<'a>(cmd: &'a str) -> Normalized<'a> {
    if cmd.len() > MAX_NORMALIZE_BYTES {
        return Normalized {
            ...
            extra_separators: SmallVec::new(),
        };
    }

    let has_backslash = cmd.contains('\\');
    let has_ansi_c = cmd.contains("$'");
    let has_printf_hex = cmd.contains("printf") && cmd.contains(r"\x");
    let has_subst = cmd.contains("$(") || cmd.contains('`');
    let has_ifs = cmd.contains("IFS=");

    if !has_backslash && !has_ansi_c && !has_printf_hex && !has_subst && !has_ifs {
        return Normalized {
            command: Cow::Borrowed(cmd),
            extra_separators: SmallVec::new(),
        };
    ...
    let mut s = Cow::Borrowed(cmd);
    if has_backslash {
        s = join_line_continuations(s, &mut budget);
    }
    if has_ansi_c {
        s = decode_ansi_c(s, &mut budget);
    }
    if has_printf_hex {
        s = decode_printf_pipe_shell(s, &mut budget);
    }
    if has_subst {
        s = splice_echo_substitution(s, &mut budget);
    }
    let extra_separators = if has_ifs {
        collect_ifs_separators(&s)
    } else {
        SmallVec::new()
    };
...
fn collect_ifs_separators(s: &str) -> SmallVec<[char; 2]> {
    if !s.contains("IFS=") {
        return SmallVec::new();
    }
    let mut extra: SmallVec<[char; 2]> = SmallVec::new();
...
mod tests {
    ...
    fn collects_single_char_ifs_separator() {
        ...
        assert_eq!(n.extra_separators.as_slice(), &['X']);
    }
    ...
    fn ifs_inline_read_takes_only_value() {
        ...
        assert_eq!(n.extra_separators.as_slice(), &[',']);
    }
