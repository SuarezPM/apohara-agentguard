#!/bin/sh
#
# agentguard one-command installer (POSIX sh).
#
...
# agentguard is a security tool, a checksum mismatch ABORTS — an unverified
...
#   AGENTGUARD_PREFIX         install dir (default: ~/.local/share/agentguard)

set -eu

VERSION="${AGENTGUARD_VERSION:-0.1.0}"
BASE_URL="${AGENTGUARD_DOWNLOAD_BASE:-https://github.com/SuarezPM/agentguard/releases/download/v${VERSION}}"
PREFIX="${AGENTGUARD_PREFIX:-${HOME}/.local/share/agentguard}"
...
Install from source instead:
  ...
  artifact="agentguard-${triple}"
  url="${BASE_URL}/${artifact}"
  tmp="$(mktemp)"

  printf 'agentguard: downloading %s\n' "$url" >&2
  download "$url" "$tmp"
  ...
  bin_dir="${PREFIX}/bin"
  mkdir -p "$bin_dir"
  bin_path="${bin_dir}/agentguard"
  mv "$tmp" "$bin_path"
  chmod 0755 "$bin_path"
  printf 'agentguard: installed binary at %s\n' "$bin_path" >&2

  # --- Register the plugin/hook config. --------------------------------------
  # Place plugin.json + hooks.json next to the binary so ${CLAUDE_PLUGIN_ROOT}
  # resolves to PREFIX and the hooks invoke ${CLAUDE_PLUGIN_ROOT}/bin/agentguard.
  printf 'agentguard: fetching plugin manifest + hook config\n' >&2
  download "${BASE_URL}/plugin.json" "${PREFIX}/plugin.json" || true
  download "${BASE_URL}/hooks.json" "${PREFIX}/hooks.json" || true

  cat >&2 <<EOF
agentguard: install complete.

To enable the hook in Claude Code, install agentguard as a plugin pointing at:
  ${PREFIX}
